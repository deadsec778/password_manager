...
import bcrypt
from crypto.crypto_key import create_new_user_key
...
def admin_update_user(user_id):
    ...
    if new_password:
        ...
        cur.execute(
            "UPDATE users SET master_password_hash=%s WHERE user_id=%s",
            (hashed, user_id)
        )

        # regenerate user encryption key
        create_new_user_key(user_id)
